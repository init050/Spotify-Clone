...
class ArtistViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=True, methods=['post', 'delete'], permission_classes=[IsAuthenticated], url_path='follow')
    def follow(self, request, slug=None):
        ...
        if request.method == 'POST':
            _, created = ArtistFollower.objects.get_or_create(user=request.user, artist=artist)
            if not created:
                return Response({'detail': 'Already following this artist.'}, status=status.HTTP_400_BAD_REQUEST)
            return Response({'detail': f'Successfully followed {artist.name}.'}, status=status.HTTP_200_OK)

        elif request.method == 'DELETE':
            try:
                follower_instance = ArtistFollower.objects.get(user=request.user, artist=artist)
                follower_instance.delete()
                return Response(status=status.HTTP_204_NO_CONTENT)
            except ArtistFollower.DoesNotExist:
                return Response({'detail': 'Not following this artist.'}, status=status.HTTP_400_BAD_REQUEST)
